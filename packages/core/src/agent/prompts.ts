...
function qaMindsetRules(): string {
  return `13. QA MINDSET — ALWAYS MOVE FORWARD: Break the step instruction into sub-goals. Execute each sub-goal in order. After executing an action, MOVE to the next sub-goal — do not retry the same action, do not assert about what you just did, do not wait for visual confirmation of intermediate actions. An assert is NOT forward progress — if your reasoning identifies a next action, your output MUST be that action, not an assert about the current state. Review your previous actions in the sub-action history below — if you have already completed all parts of the step instruction, set stepComplete: true immediately instead of re-verifying or re-asserting. Example: step says "select Satellite and dismiss the panel" → sub-goal 1: tap Satellite, sub-goal 2: tap X to close. After tapping Satellite, immediately tap X — do not assert that Satellite is selected. After all sub-goals are done, set stepComplete: true.
   PROACTIVE ADAPTABILITY: If a step or requirements context is high-level, unclear, or expects a state that is not yet met (e.g., verifying a completed form while the form is empty, or navigating to a page that is not visible), do NOT fail the step immediately. Instead, analyze the current screen, determine the missing steps or inputs required to fulfill the step's intent, and perform the actions (e.g., fill the form, click inputs, trigger navigation) to move the flow forward rather than failing early.
...
function decompositionRules(): string {
  return `DECOMPOSITION CHECK: Before every action, silently identify the ordered checklist for the current step. Treat "and", "then", "after", commas, bullets, form fields, wizard screens, and visible required fields as separate sub-goals. If the page structure reveals a prerequisite not named in the step (cookie dialog, required field, disabled submit, intermediate Next/Continue, modal, tab, accordion, or dropdown), handle that prerequisite before the requested final action. Do not set stepComplete until every explicit sub-goal and every discovered prerequisite needed for the step has been handled or verified from the current screen and sub-action history.`
}
...
function appendStepDecompositionGuide(parts: string[]): void {
  parts.push('')
  parts.push('Step decomposition guardrail:')
  parts.push('- Work the current step as an ordered checklist, not as one broad intent.')
  parts.push('- Split compound wording and visible page prerequisites into separate sub-goals.')
  parts.push('- Use the current page structure to discover required inputs, consent dialogs, intermediate Continue/Next actions, tabs, accordions, dropdowns, and modals.')
  parts.push('- Only set stepComplete when the current screen plus sub-action history show the whole checklist is done.')
}
...
export function buildStepPrompt(
  ...
  appendStepDecompositionGuide(parts)
  parts.push('')
