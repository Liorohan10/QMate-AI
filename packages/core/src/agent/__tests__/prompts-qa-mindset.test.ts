...
describe('QA mindset prompt rules', () => {
  ...
  describe('AGENT-QA-01 — always move forward', () => {
    ...
    it('buildSystemPrompt requires decomposition and discovered prerequisites', () => {
      expect(systemPrompt).toContain('DECOMPOSITION CHECK')
      expect(systemPrompt).toContain('visible required fields as separate sub-goals')
      expect(systemPrompt).toContain('discovered prerequisite')
      expect(systemPrompt).toContain('Do not set stepComplete until every explicit sub-goal')
    })
  })
  ...
  describe('AGENT-LOOP-01 — step completion recognition', () => {
    ...
    it('buildStepPrompt includes the step decomposition guardrail near current step', () => {
      const state: ScreenState = {
        tree: 'input "Email" [ref=e1]\nbutton "Continue" [ref=e2]',
        elements: [],
        url: '',
        timestamp: 0,
        metadata: { coordSpace: 'viewport' as const, viewportWidth: 0, viewportHeight: 0 },
      ...
      const context: StepContext = {
        stepInstruction: 'Fill the form and continue',
        testName: 'test',
        previousSteps: [],
        plannerModel: {} as any,
        verifierModel: {} as any,
      ...
      const prompt = buildStepPrompt('Fill the form and continue', state, context)
      expect(prompt).toContain('Step decomposition guardrail:')
      expect(prompt).toContain('ordered checklist')
      expect(prompt).toContain('visible page prerequisites')
      expect(prompt).toContain('Only set stepComplete')
    })
  })
})
