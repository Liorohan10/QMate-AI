...
export default function TestEditorPage() {
  ...
  const handleImportRequirements = async () => {
    ...
    setIsImporting(true)
    try {
      const response = await fetch('/api/tests/generate-from-requirements', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to import requirements')
      }
          ...
          <div className="rounded-md border bg-muted/15 px-3 py-3">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <select
                  value={importSource}
                  onChange={(e) => setImportSource(e.target.value as 'confluence' | 'jira')}
                  className="rounded border border-input bg-background h-8 px-2 py-1 text-xs outline-none focus-visible:ring-1 focus-visible:ring-ring"
                >
              ...
              <Input
                placeholder={importSource === 'confluence' ? "Page ID (e.g. 12345)" : "Jira Issue Key (e.g. PROJ-123)"}
                value={importId}
