      ...
      try {
        ...
        const runOneDirectTest = async (test: TestDefinition): Promise<DirectTestOutcome> => {
          ...
          const adapterKey = JSON.stringify({
            ...
            const verifierModel = await createModel(verifierModelConfig)
            const plannerConfig = {
              // Match live session mode: 50 sub-actions per step allows the agent to
              // handle complex multi-interaction UI patterns (autocomplete, modals, etc.)
              // without hitting the limit that causes premature step failure.
              maxSubActions: testMergedConfig.use?.planner?.maxSubActions ?? 50,
              previousStepCount: testMergedConfig.use?.planner?.previousStepCount ?? 5,
            }
