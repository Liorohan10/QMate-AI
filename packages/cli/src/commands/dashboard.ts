...
async function startDashboardBackedServices(
  ...
  try {
    ...
    const { close } = await startServer({
      ...
      authFetch,
      // Pass the absolute path to the running CLI script so the TestRunner
      // can spawn child processes reliably on Windows without shell:true.
      cliBin: process.argv[1] ? resolve(process.argv[1]) : undefined,
    })
