...
export class DashboardReporter implements Reporter {
  ...
  private async persistStep(step: StepResult, runId: string, stepOrder: number): Promise<void> {
    ...
    const isStepHealed = (() => {
      if (safeStep.status !== 'passed') return false
      const subActions = safeStep.trace?.subActions ?? []
      ...
      if (hasDuplicateRefs) return true

      // 4. Assertion step with preceding interactive actions (e.g. clicks/fills)
      const isAssertStep = /^(verify|assert|check|confirm|should|page shows)/i.test(safeStep.name)
      ...
      if (isAssertStep && hasInteractiveActions) return true

      // 5. Click step with multiple interactive actions (e.g. clicked wrong thing first)
      const isClickStep = /^(click|tap|press|select)/i.test(safeStep.name)
      if (isClickStep) {
        const interactiveCount = subActions.filter((sa: any) => {
          const type = sa.plannedAction?.type || sa.action?.type
          return ['click', 'fill', 'tap', 'type'].includes(type)
        }).length
        if (interactiveCount > 1) return true
      }
    ...
    const stepId = this.db.insertStep({
      ...
      runId,
      name: safeStep.name,
      status: isStepHealed ? 'healed' : safeStep.status,
