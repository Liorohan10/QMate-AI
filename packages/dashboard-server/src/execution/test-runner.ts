...
export class TestRunner extends EventEmitter implements ExecutionBackend {
  ...
  static resolveCliBin(): string {
    ...
    if (process.platform === 'win32') {
      try {
        const where = execSync('where agent-qa.cmd', { encoding: 'utf-8' }).split('\n')[0]?.trim()
        if (where) return where
      } catch {
        // not found in PATH
      }
      return '.\\node_modules\\.bin\\agent-qa.cmd'
    }
  ...
  execute(opts: ExecuteOptions): void {
    ...
    // Determine how to spawn the CLI on this platform:
    // - If cliBin is a .js file (run via `node cli.js dashboard`), spawn with `node` directly.
    // - On Windows, .cmd wrappers need cmd.exe /c.
    // - On Unix, spawn directly with detached process group.
    let child: ReturnType<typeof spawn>
    const isJsScript = this.cliBin.endsWith('.js') || this.cliBin.endsWith('.mjs')
    if (isJsScript) {
      // Spawn as: node <cli.js> run <args...> — works on all platforms without shell
      child = spawn(process.execPath, [this.cliBin, ...spawnArgs], {
        stdio: ['ignore', 'pipe', 'pipe'],
        env,
        detached: process.platform !== 'win32',
      })
    } else if (process.platform === 'win32') {
      // Windows: .cmd files cannot be spawned directly (EINVAL), they need cmd.exe
      // Using cmd.exe /c avoids DEP0190 (shell:true + args array)
      child = spawn('cmd.exe', ['/c', this.cliBin, ...spawnArgs], {
        stdio: ['ignore', 'pipe', 'pipe'],
        env,
      })
    } else {
      child = spawn(this.cliBin, spawnArgs, {
        stdio: ['ignore', 'pipe', 'pipe'],
        env,
        detached: true,
      })
