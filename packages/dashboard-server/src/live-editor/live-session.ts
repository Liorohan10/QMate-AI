        ...
        if (maxHealingAttempts > 0) {
          ...
          try {
            const recoveryLoopConfig = {
              ...loopConfig,
              onPhase: (event: any) => {
                // Log healing phases inside live session but do not report them as main UI steps
                console.log(`[live-session] [Healing Phase] ${event.phase}: ${event.text ?? ''}`)
              }
            }
            ...
            if (recoveryResult.status === 'passed') {
              ...
              const retryResult = await executeStep(prepared.instruction, loopConfig, retryContext) as LiveStepExecutionResult
              ...
              result = recoveryResult as LiveStepExecutionResult
            }
          } catch (healErr) {
