...
export class DashboardDatabase {
  ...
  getHealedRunIds(runIds: string[]): Set<string> {
    ...
    const healedIds = new Set(rows.map((row) => row.run_id))

    // Fallback: scan sub_actions_data for older runs where step status was recorded as 'passed'
    try {
      const steps = this.db.prepare(`
        SELECT run_id, name, sub_actions_data
        FROM steps
        WHERE run_id IN (${placeholders})
          AND sub_actions_data IS NOT NULL
      `).all(...runIds) as Array<{ run_id: string; name: string; sub_actions_data: string }>

      for (const step of steps) {
        if (healedIds.has(step.run_id)) continue
        try {
          const subActions = JSON.parse(step.sub_actions_data)
          if (Array.isArray(subActions) && subActions.length > 0) {
            // 1. Any execution failure
            const hasFailed = subActions.some((sa: any) => sa.result === 'failure')
            if (hasFailed) {
              healedIds.add(step.run_id)
              continue
            }
            // 2. Any verifier rejection on a non-final action
            if (subActions.length > 1) {
              const hasRejected = subActions.slice(0, -1).some((sa: any) => sa.verifierReasoning !== undefined)
              if (hasRejected) {
                healedIds.add(step.run_id)
                continue
              }
            }
            // 3. Repeated actions on the same element/reference
            const refs = subActions.map((sa: any) => sa.plannedAction?.ref || sa.action?.ref).filter(Boolean)
            const hasDuplicateRefs = refs.some((ref: string, idx: number) => refs.indexOf(ref) !== idx)
            if (hasDuplicateRefs) {
              healedIds.add(step.run_id)
              continue
            }
            // 4. Assertion step with preceding interactive actions (e.g. clicks/fills)
            const isAssertStep = /^(verify|assert|check|confirm|should|page shows)/i.test(step.name || '')
            const hasInteractiveActions = subActions.slice(0, -1).some((sa: any) => {
              const type = sa.plannedAction?.type || sa.action?.type
              return ['click', 'fill', 'tap', 'type', 'hover'].includes(type)
            })
            if (isAssertStep && hasInteractiveActions) {
              healedIds.add(step.run_id)
              continue
            }
            // 5. Click step with multiple interactive actions
            const isClickStep = /^(click|tap|press|select)/i.test(step.name || '')
            if (isClickStep) {
              const interactiveCount = subActions.filter((sa: any) => {
                const type = sa.plannedAction?.type || sa.action?.type
                return ['click', 'fill', 'tap', 'type'].includes(type)
              }).length
