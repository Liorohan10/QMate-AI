...
interface StartServerOptions {
  ...
  /** Absolute path to the agent-qa CLI binary. When omitted, TestRunner.resolveCliBin() is used. */
  cliBin?: string
}
...
export async function startServer(opts: StartServerOptions): Promise<{
  ...
  const { db, port = 3470, uiDir, artifactsDir, workspacePaths, configPath, mcp, llmConfig, authFetch, cliBin: cliBinOpt } = opts
  ...
  try {
    cliBin = cliBinOpt ?? TestRunner.resolveCliBin()
  } catch {
